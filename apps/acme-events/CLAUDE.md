# Acme Events — Claude Code project rules
...
## Known bugs (planted for demo)

Bug 1 — Stripe idempotency (`server/main.py: create_payment_intent`)
: `stripe.PaymentIntent.create` is called without an `idempotency_key`.
  Network retries create duplicate PaymentIntents → double charge.

Bug 2 — Surge webhook auth (`server/main.py: surge_webhook`)
: No signature verification on the `surge_signature` header.
  Unauthenticated callers can POST fake `message.delivered` events.

Demo prompt: `./fetchsandbox customers getting double-charged on retry and someone is faking our SMS delivery events`
