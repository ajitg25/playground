...
6. **Final summary** surfaces the brain's full `check_for` items as
   the audit checklist. Items you can't resolve in-scope get flagged
...
> question. Example: `./fetchsandbox why are pro users not getting their role after payment?`
...
- Inline-code only for literal tokens (e.g. `u.get("stripe_id")`).
- No "I'll now apply the fix" preambles. Just apply.

## Why these rules exist

This dir is a recording-ready cross-service integration (Clerk + Stripe +
Resend). The product being demonstrated (FetchSandbox) carries a brain that
encodes API-specific failure modes across service boundaries. The dispatch
convention guarantees the brain is consulted on every task. Local-only tests
defeat the demonstration — the brain's value compounds across customers, and
that compounding only happens if every task routes through it.
